...
void* thread_routine (void *arg) {
	...
	thread_result *thread_res = \
		(struct thread_result*)calloc(1, sizeof(struct thread_result*));
...
int main (int argc, void *argv[]) {
	...
	for (thread_count = 0; thread_count < THREADS; thread_count++) {
		...
		status = pthread_join (thread[thread_count].thread_id, thread_res);
		if (status != 0)
			err_abort (status, "Join thread");

		if (thread_res[thread_count].printed == true)
		{
