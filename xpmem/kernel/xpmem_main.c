...
xpmem_init(void)
{
	...
	for (i = 0; i < XPMEM_TG_HASHTABLE_SIZE; i++) {
		xpmem_my_part->tg_hashtable[i].lock = RW_LOCK_UNLOCKED;
	...
	global_pages_entry->write_proc = xpmem_unpin_procfs_write;
	global_pages_entry->read_proc = xpmem_unpin_procfs_read;
	//global_pages_entry->owner = THIS_MODULE;
	global_pages_entry->uid = current->cred->uid;
	global_pages_entry->gid = current->cred->gid;
	...
	debug_printk_entry->data = 0;
	debug_printk_entry->write_proc = xpmem_debug_printk_procfs_write;
	debug_printk_entry->read_proc = xpmem_debug_printk_procfs_read;
	//debug_printk_entry->owner = THIS_MODULE;
